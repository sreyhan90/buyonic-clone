import { monitorType, winterColletionType } from "@/app/constants";
...
const MonitorList = ({ productMonitor }: MonitorProps) => {
  const { addToCart } = useCart();
  const discount = Math.round(
    ((productMonitor.mrp - productMonitor.price) / productMonitor.mrp) * 100,
  );
  return (
    <div>
      <div>
        <div className="p-4">
          <Card className="group mx-auto w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="relative flex h-56 items-center justify-center rounded-xl bg-gray-50 p-4">
              ...
              <img
                src={productMonitor.image}
                alt={productMonitor.title}
            ...
            <CardFooter className="px-0 pb-0 pt-2">
              <Button
                ...
                onClick={() => {
                  addToCart({
                    id: productMonitor.id,
                    title: productMonitor.title,
                    price: productMonitor.price,
                    image: productMonitor.image,
                  });
