...
interface PowerBanksProps {
  productpowerBank: powerBankType;
}

const PowerBanksList = ({ productpowerBank }: PowerBanksProps) => {
  const { addToCart } = useCart();
  const discount = Math.round(
    ((productpowerBank.mrp - productpowerBank.price) / productpowerBank.mrp) *
      100,
  );
  return (
    <div>
        ...
        <div className="p-4">
          <Card className="group mx-auto w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="relative flex h-56 items-center justify-center rounded-xl bg-gray-50 p-4">
              ...
              <img
                src={productpowerBank.image}
                alt={productpowerBank.title}
            ...
            <div className="mb-3 flex items-center gap-3 text-left">
              <span className="text-2xl font-extrabold text-gray-900">
                ${productpowerBank.price}
              </span>
              <span className="text-sm font-medium text-gray-400 line-through">
                ${productpowerBank.mrp}
              </span>
            </div>
            ...
            <CardFooter className="px-0 pb-0 pt-2">
              <Button
                ...
                onClick={() => {
                  addToCart({
                    id: productpowerBank.id,
                    title: productpowerBank.title,
                    price: productpowerBank.price,
                    image: productpowerBank.image,
                  });
