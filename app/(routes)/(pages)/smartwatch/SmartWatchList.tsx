import { smartWatchType } from "@/app/constants";
...
interface smartWatchProps {
  productSmartWatch: smartWatchType;
}

const SmartWatchList = ({ productSmartWatch }: smartWatchProps) => {
  const { addToCart } = useCart();
  const discount = Math.round(
    ((productSmartWatch.mrp - productSmartWatch.price) /
      productSmartWatch.mrp) *
      100,
  );
  return (
    <div>
        ...
        <div className="p-4">
          <Card className="group mx-auto w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <div className="relative flex h-56 items-center justify-center rounded-xl bg-gray-50 p-4">
              ...
              <img
                src={productSmartWatch.image}
                alt={productSmartWatch.title}
            ...
            <div className="mb-3 flex items-center gap-2 text-left">
              <div className="flex items-center gap-1 text-sm font-semibold text-green-600">
                <span>{productSmartWatch.rating}</span>
                <StarIcon className="h-4 w-4 fill-green-600 text-green-600" />
              </div>
              <span className="text-sm text-gray-500">
                ({productSmartWatch.ratingCount})
              </span>
            </div>
            ...
            <CardFooter className="px-0 pb-0 pt-2">
              <Button
                ...
                onClick={() => {
                  addToCart({
                    id: productSmartWatch.id,
                    title: productSmartWatch.title,
                    price: productSmartWatch.price,
                    image: productSmartWatch.image,
                  });
