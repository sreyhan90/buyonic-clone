import { EyeWearType, LaptopType } from "@/app/constants";
...
interface EyeWearProps {
  productEyeWear: EyeWearType;
}

const EyeWearListpage = ({ productEyeWear }: EyeWearProps) => {
  const { addToCart } = useCart();
  const discount = Math.round(
    ((productEyeWear.mrp - productEyeWear.price) / productEyeWear.mrp) * 100,
  );
  return (
    <div>
      <div>
          ...
          <Card className="group mx-auto w-full max-w-sm rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
            ...
            <CardFooter className="px-0 pb-0 pt-2">
              <Button
                ...
                onClick={() => {
                  addToCart({
                    id: productEyeWear.id,
                    title: productEyeWear.title,
                    price: productEyeWear.price,
                    image: productEyeWear.image1,
                  });
...
export default EyeWearListpage;
