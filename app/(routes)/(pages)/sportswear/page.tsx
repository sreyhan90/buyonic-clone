"use client";
import { productwinter } from "@/app/constants";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { useMemo, useState } from "react";
import { ArrowLeft } from "lucide-react";
import WinterCollectionList from "./WinterCollectionList";
...
const sportswear = () => {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let updatedProducts = [...productwinter];
        ...
        <div className="min-h-screen bg-white">
          <div className="mx-auto max-w-7xl px-6 py-6">
            ...
            <div className="mb-4 flex items-center gap-2 text-sm font-medium text-gray-600">
              ...
              <span className="font-medium text-gray-900">Sportswear</span>
            </div>
            ...
            <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-gray-500">
                  Discover performance-driven apparel designed for comfort,
            ...
            <div className="grid grid-cols-1 gap-1 sm:grid-cols-2 lg:grid-cols-3">
              {filteredAndSortedProducts.map((product) => (
                <WinterCollectionList
                  key={product.id}
                  productWinter={product}
                />
              ))}
            </div>
          </div>
